...
// no direct access
	...
	if (isset($_REQUEST['prod_id']))	
	{
		...
		$sql_statement = 'SELECT cat_id FROM sweeps_cat_asign WHERE prod_id = "'.$prod_id.'"';
	...
	if (isset($cat_id))
	{
		...
		$sweeps_cat_result = mysql_fetch_array (ReadDB ($sql_statement, $mysql_err_msg));
		
		//	determine if there is only one parent CAT
		if ($cat_id == 0)
		{$sweeps_num_cats = mysql_num_rows (ReadDB ($sql_statement, $mysql_err_msg));}
		else {$sweeps_num_cats = 2;}	//	set $sweeps_num_cats greater than 1 to trigger other events
		
		$sweeps_cats_name = $sweeps_cat_result['cat_name'];
		
	}
	
	
	echo "\n";			
	echo TAB_7.'<!--	START Online Sweeps Shop code 	-->'."\n";
	echo "\n";
	...
	{ include_once ('sweeps/sweeps_breadcrumb.php'); }	
	...
	echo "\n";		
		...
		//	 Do  plug in Module s for the sweeps MOD's Left column
	...
	echo TAB_7.'<div class="ShopDiv" id="ShopCentreColumn" >'."\n";
		...
		//	determin what to display:	PRODUCT BROWSING / CART / CHECKOUT / PAYMENT MADE PAGE

		if ($view_set == 'paymade' AND isset($_REQUEST['invoice']) AND $_REQUEST['invoice'] != '')
		{ 
			//	Show order confirmation details	
			require ('sweeps/sweeps_receipt_display.php');	
	
			include ('sweeps/sweeps_item_suggest.php');	
					
		}		
				
		elseif ($view_set == 'cart' AND isset($_SESSION['cart_items'])) 	
		{
			require ('sweeps/sweeps_cart_view.php');
			include ('sweeps/sweeps_item_suggest.php');			
		}
		...
		{ 
			...
			{
				require ('sweeps/sweeps_checkout.php');				
			}
		...
		{ 
			require ('sweeps/sweeps_item_browser.php');
		}
		...
		{ 
			...
			{
				require ('sweeps/sweeps_cart_view.php');
				require ('sweeps/sweeps_item_suggest.php');	
			}
			
			//	no items in cart. browse
			else {require ('sweeps/sweeps_item_browser.php');}		
	...
	//	right side colunm
	echo TAB_7.'<div id="ShopRightColumn" >'."\n";
	echo "\n";		
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	echo "\n";			
	echo TAB_7.'<!--	END Online Sweeps Shop code 	-->'."\n";
	echo "\n";		
