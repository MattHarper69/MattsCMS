...
// no direct access
	...
	$shop_opened_link_ids = GetShopCatLinkPath ($cat_id);

	//	Reverse array and add current cat to array
	$shop_opened_link_ids = array_reverse($shop_opened_link_ids);
	$shop_opened_link_ids[] = $cat_id;
		...
		echo TAB_7.'<div class="ShopDiv" id="ShopBreadCrumb" >'."\n";					
			...
			echo TAB_9.'<a class="BreadCrumb" href="'.$href.'" title ="Return to start of the Catalogue" >Home </a>'."\n";				
							
			
			foreach ($shop_opened_link_ids as $link_id)
			{
							
				if ($link_id != "" AND $link_id != 0 )
				{
					$mysql_err_msg = 'The Menu for this page is unavailable';
					$sql_statement = 'SELECT * FROM shop_categories WHERE cat_id = "'.$link_id.'" ';
...
//	===========================================================================================================================
	...
	function GetShopCatLinkPath ($cat_id)
	{

		$shop_opened_link_ids = array();
		$current_id = $cat_id;
		...
		for ( $i = 0; $i < (NUM_NAV_LAYERS - 1); $i++ )
		{
			...
			$mysql_err_msg = 'The Menu for this page is unavailable';
			$sql_statement = 'SELECT parent_id, cat_id FROM shop_categories WHERE active = "on" '
																			.'AND cat_id = "'.$current_id.'"';
				

			$cat_nav_row = mysql_fetch_array (ReadDB ($sql_statement, $mysql_err_msg));
			
			$shop_opened_link_ids[] = $cat_nav_row['parent_id'];
			
			$current_id = $cat_nav_row['parent_id'];
			
		}
		
		return $shop_opened_link_ids;
